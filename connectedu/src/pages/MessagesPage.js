...
const MessagePage = () => {
  ...
  const handleComposeSubmit = async (values) => {
    try {
      // Envoi d'un seul mail
      const response = await axios.post(
        "http://192.168.1.3:8000/contact/send-single-email/",
        {
          subject: values.subject,
          message: values.message,
          recepteur: [values.recipient], // Envoyer à un seul destinataire
        }
      );
        ...
        <Form onFinish={handleComposeSubmit}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Veuillez entrer l'adresse e-mail du destinataire",
              },
            ]}
          >
          ...
          <Form.Item
            ...
            name="subject"
            rules={[
              { required: true, message: "Veuillez entrer l'objet du message" },
            ]}
          >
